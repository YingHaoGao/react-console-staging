    ...
    return (
      <div className="tableBox">
        <table>
          ...
          <tbody>
            <tr>
              <td className="borR" rowSpan="4">{__('presales-manpower-cost')}</td>
              <td className="borD">
                <div>{__('presales-before-bidding')}</div>
            ...
            <tr>
              <td>
                <div>{__('presales-delivery-input')}</div>
                {isEdit ? (<Input />) : (info.name)}
              </td>
              <td>
                {isEdit ? (<Input />) : (info.name)}
            ...
            <tr>
              <td>
                <div>{__('presales-after-bidding')}</div>
                {isEdit ? (<Input />) : (info.name)}
              </td>
              <td>
                {isEdit ? (<Input />) : (info.name)}
            ...
            <tr>
              <td>
                <div>{__('presales-delivery-bidding')}</div>
                {isEdit ? (<Input />) : (info.name)}
              </td>
              <td>
                {isEdit ? (<Input />) : (info.name)}
          ...
          <tbody>
            <tr>
              <td className="borR" rowSpan="4">{__('presales-personnel-budget')}</td>
              <td className="borD">
        ...
        <h2>{__('presales-total-cost')}:&ensp;{info.name}</h2>
      </div>
    )
  }
}
